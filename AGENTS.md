...
- The safest current positioning is: KCH-MedRank is a white-box biomedical evidence reranking / evidence-control layer for medical RAG. The dominant measured performance drivers are enhanced high-recall candidate generation, biomedical semantic retrieval or reranking signals, and query-group supervised learning-to-rank. Hypergraph, MeSH, entity, and relation features should be described as interpretable complementary signals unless new evidence proves a larger independent effect.
...
- Before claiming runtime or efficiency advantages over MedCPT Cross-Encoder, verify whether KCH-MedRank uses any Cross-Encoder score as a test-time feature. If it uses a Cross-Encoder feature online, include Cross-Encoder scoring time in the KCH total. If the semantic feature is a MedCPT dense / dual-encoder score or another precomputed source, state that explicitly and do not call it a Cross-Encoder feature.
- Add or maintain a fair efficiency table separating: MedCPT Cross-Encoder online scoring, KCH-MedRank with available semantic features, KCH-MedRank without Cross-Encoder / without semantic features, and retrieval-feature-only LambdaMART. Any speedup claim must state whether first-stage retrieval, semantic scoring, feature construction, model loading, training, and LightGBM scoring are included.
- Add a no-Cross-Encoder-feature or no-semantic-feature KCH-MedRank diagnostic before using efficiency as a main claim. This diagnostic should report Recall@10, MRR@10, nDCG@10, and reranking time.
- Add at least one external evidence-retrieval dataset or official retrieval split before external submission if feasible. Prefer official BioASQ snippet/document retrieval, BEIR biomedical subsets such as NFCorpus or TREC-COVID, or PubMedQA retrieval-only diagnostics. This does not require a new LLM generation study.
...
- Current PubMedQA Qwen3-8B generation pilot should remain an appendix or diagnostic result unless a controlled full generation loop shows stronger answer accuracy, citation support, or unsupported-claim improvements. Do not use weak or small generation results as a primary contribution.
