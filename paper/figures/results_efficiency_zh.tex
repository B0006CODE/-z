...
\begin{scope}
  ...
  \node[anchor=west, font=\bfseries] at (-0.35,4.45) {BioASQ 留出测试 top-10 证据指标};
  ...
  \node[lab, anchor=west] at (0.2,-0.72) {\textcolor{kchA}{\rule{0.28cm}{0.12cm}} 原始 Hybrid};
  \node[lab, anchor=west] at (2.25,-0.72) {\textcolor{kchB}{\rule{0.28cm}{0.12cm}} 增强 Hybrid};
...
\begin{scope}[shift={(9.8,0)}]
  ...
  \node[anchor=west, font=\bfseries] at (-0.35,4.45) {重排序阶段开销};
  ...
  \node[lab, anchor=west] at (0.26,0.85) {KCH-MedRank 表格排序器：1.0$\times$};
  \node[lab, anchor=west] at (3.14,2.15) {MedCPT Cross-Encoder：19.3$\times$};
  \node[lab, align=left, anchor=west] at (0,3.25) {同一增强候选池；\\相对重排序开销来自\\留出测试计时比较。};
\end{scope}
\end{tikzpicture}}
\caption{本文主量化结果图。KCH-MedRank 在 BioASQ 留出测试 top-10 证据指标上优于 Hybrid RRF 和 MedCPT Cross-Encoder 排序，同时在同一候选池上的表格重排序阶段显著低于完整 cross-encoder 重打分开销。}
