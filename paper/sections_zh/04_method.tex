...
\input{figures/hypergraph_schema_zh}
...
\begin{enumerate}
  ...
  \item \textbf{段落特征抽取。} 对每个段落拼接检索分数与排名、生物医学语义分数、实体重叠、MeSH 精确与层级特征、辅助关系特征、扩散分数、中心性和局部图计数特征。
...
主监督重排序器采用查询分组学习排序模型，即使用 LambdaMART 风格目标训练的 LightGBM 梯度提升树排序器 \citep{ke2017lightgbm}。每个问题构成一个排序组，候选段落构成样本行，二值相关性标签来自金标准证据标注。特征向量结合检索分数、生物医学语义分数、实体特征、精确与层级感知 MeSH 特征、辅助关系特征和局部超图结构特征。
